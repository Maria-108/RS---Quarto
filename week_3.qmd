...
Ilori et al. (2019) conducted a study where they set out to investigate atmospheric correction techniques particularly for coastal remote sensing cases. They aimed to extract the ocean colour which *"provides information on in-water optical properties"* (pg 1), however this is difficult to extract as only "\~10% of the total signal reaches the TOA" (pg 2). The main method that was applied to this study was atmospheric correction, but the researchers recognised that *"residual errors can introduce large uncertainties in R~rs~ estimates, resulting in erroneous retrieval of OC products such as apparent optical properties"* (pg 2).
